...
const MyBookmarks = () => {
  ...
  return (
    <>
      ...
      <main>
        {bookmarkIsEmpty && <p style={{ fontFamily: 'Open Sans'}}>You have not bookmarked any news yet.</p>}
        {!bookmarkIsEmpty &&
          myBookmarks.map((myBookmark, index) => {
            return (
              <div className={styles.content_wrapper} key={index}>
                <p >{myBookmark.displayUrl}</p>
                <a href={myBookmark.url}><h3>{myBookmark.name}</h3></a>
                <p>{myBookmark.snippet}</p>
                <p style={{ fontSize: "0.75rem"}}>{formatDate(myBookmark.dateLastCrawled)}</p>
                <div className={styles.bookmark_wrapper} onClick={() => removeHandler(myBookmark)}>
                  <span className="material-symbols-outlined NormalResult_bookmark__pGo9r">
                    bookmark_remove
                  </span>
                  <p style={{ fontSize: "0.875rem", fontFamily: 'Open Sans' }}>Remove Bookmark</p>
                </div>
              </div>
            );
          })}
